...
# PS00399_d35_3_to_5 = sum(ModelData$conc >= 3 & ModelData$conc <= 5)/10000
# PS00399_d35_over_5 = sum(ModelData$conc>5)/10000
# PS00399_d35 = ModelData

ModelData$color<-ifelse(test = ModelData$conc>=5,yes = "Above 5 log",no = 
                    ifelse(ModelData$conc>=3,yes = "Between 3 and 5 log",no = "Below 3 log"))
breaks <- seq(0, 10, by = 0.5)
finalhist<-ggplot(data = ModelData,aes(x = conc))
finalhist<-finalhist+
  geom_histogram(data = ModelData,aes(fill=color),binwidth = 0.1, breaks = breaks)+
...
files <- c(
  "pseudomycoides_d21.tiff",
  "albus_d35.tiff",
  "mobilis_d35.tiff",
  "tropicus_d35.tiff",
  "pacificus_d35.tiff",
  "cereus_d35.tiff",
  "thuringiensis_d35.tiff",
  "toyonensis_d35.tiff",
  "cytotoxicus_d35.tiff"
)

imgs <- lapply(files, magick::image_read)

plots <- lapply(imgs, function(img) {
  cowplot::ggdraw() + cowplot::draw_image(img)
})

combined_plot_d35 <- cowplot::plot_grid(plotlist = plots, ncol = 3)
combined_plot_d35
